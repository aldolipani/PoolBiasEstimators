...
class DepthNPool(val n: Int, lRuns: List[Runs], gT: QRels) extends Pool(lRuns, gT) {
  ...
  //override def getPooledDocuments(topicId: Int): Set[Document] = DepthNPool.getPooledDocuments(n, lRuns, gT)(topicId)
...
object DepthNPool {
  ...
  def getPooledDocuments(n: Int, lRuns: List[Runs], qRels: QRels)(topicId: Int): Set[Document] =
  /*PoolAnalyzer.fixRuns(*/ lRuns /*, qRels)*/ .flatMap(l => {
